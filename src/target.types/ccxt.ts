...
export default class ccxtConnection extends BaseConnection {
        ...
        if (!this.initialized || forceReload) {
            ...
            if (this.access === "private") {
                ...
                const quotes: Array<string> = _.uniq(
                    this.markets.map(
                        (currencyPair: string) => currencyPair.split("/")[1]
                    )
                );
        ...
        if (this.access === "private") {
            ...
            if (since) {
                withdrawals = withdrawals.filter(
                    (deposit: Transaction) =>
                        deposit.timestamp.getTime() > since
                );
            }
            if (this.requireUsdValuation) {
    ...
    async getDeposits(
        ...
        if (this.access === "private") {
            ...
            if (this.requireUsdValuation) {
                ...
                deposits = deposits.filter(
                    (tx: Transaction) => tx.baseUsdPrice !== 0
                );
            }
            return deposits;
        }
    ...
    async getLedger(
        ...
        if (this.access === "private") {
            ...
            const ledger = await this.connection.fetchLedger(
                symbol,
                since,
                limit
            );
                    ...
                    while (x < 3) {
                        try {
                            ...
                            if (
                                ...
                                await sleep(1500);
                                console.log(
                                    "rate limit exceed...sleeping",
                                    e.message
                                );
                                x++;
                            } else {
                                console.log(
                                    `getTransactions failed => ${e.message}`
                                );
                                break;
                            }
                        }
    ...
    async quoteTransaction(
        ...
        if (!prices) {
            prices = await this.getCommonPrices(
